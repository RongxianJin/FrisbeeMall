...
async function loadCurrentProduct() {
    try {
        const response = await axios.get(`${API_BASE_URL}/product/show`);
...
async function submitBuyerInfo(e, productId) {
    ...
    const buyerIdError = document.getElementById('buyerIdError');
    
    if (isNaN(buyerId)) {
        buyerIdError.textContent = '请输入有效的数字ID';
        return;
    } else {
        buyerIdError.textContent = '';
    }
